...
**Update, 21 Sep 2026:** CISA added this CVE to its Known Exploited Vulnerabilities catalog in early September 2026 alongside CVE-2026-45312 (case 071), citing active campaigns against exposed RAGFlow instances that gain admin access, deploy reverse shells and cryptominers, and steal LLM provider API keys; federal agencies were given until 05 Sep 2026 to remediate. RAGFlow's v0.25.0 release (21 Apr 2026) is credited with broadly adopting `jinja2.sandbox.SandboxedEnvironment` for template rendering, but the GHSA-vvwj-fvwh-4whx advisory itself still lists no patched version, and it's not independently confirmed here that the fix reaches both `string_transform.py` and `message.py` specifically. Given confirmed active exploitation, treat this as unresolved rather than patched until the advisory itself is updated.
