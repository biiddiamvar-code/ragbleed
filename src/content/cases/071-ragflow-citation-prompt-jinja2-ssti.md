...
**Update, 21 Sep 2026:** CISA added this CVE to its Known Exploited Vulnerabilities catalog in early September 2026 alongside CVE-2026-28797 (case 091), citing active campaigns against exposed RAGFlow instances that gain admin access, deploy reverse shells and cryptominers, and steal LLM provider API keys; federal agencies were given until 05 Sep 2026 to remediate. Separately, RAGFlow's `main` branch now builds `rag/prompts/generator.py`'s Jinja2 environment with `jinja2.sandbox.SandboxedEnvironment`, a change tied to the v0.25.0 release (21 Apr 2026) — but the GitHub security advisory itself still lists no patched version, so this is a fix visible in the code, not a confirmed, formally patched release. Given confirmed active exploitation, treat any pre-0.25.0 instance as compromised-until-proven-otherwise, not merely at risk.
