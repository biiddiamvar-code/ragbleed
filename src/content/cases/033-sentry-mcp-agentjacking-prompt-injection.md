...
**Update, 21 Sep 2026:** Sentry has not shipped a fix for the underlying trust boundary, but its `sentry-mcp` repository shows continued incremental mitigation work: PR #1284 (merged 08 Sep 2026) changes `get_issue_details` to return structured content, and open issues/PRs (#1093, #1056, #1132) track exposing event provenance and closing remaining untrusted-data bypasses. None of this closes the architectural gap described above — the status here remains accurate.
